  ...
  return Object.freeze({
    async submit(request, rawToken) {
      ...
      try {
        ...
        return response(result.status, result.body, result.replayed ? { "Idempotency-Replayed": "true" } : {});
      } catch (error) {
        ...
        if (error instanceof GuestBookingRequestError) {
          const status = error.code === "invalid_share_link" ? 404
            : ["slot_unavailable", "bookings_closed", "booking_window_unavailable"].includes(error.code) ? 409
              : 400;
