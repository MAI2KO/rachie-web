...
export function BookingExperience({ brand }: { brand: ActiveBrand }) {
  ...
  async function submitBooking(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setConfirmation(result.booking); setSuccess(`${result.booking.serviceLabel} booked for ${result.booking.date} at ${result.booking.displayTime}.`); setSelectedSlot(""); setRequirements({}); await Promise.all([loadBookingData(), loadAvailability(selectedService)]); queueMicrotask(() => successRef.current?.focus());
